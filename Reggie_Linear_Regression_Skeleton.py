...
# +
def get_y(m: float, b: float, x: float) -> None:
    return (m * x) + b  # type: ignore
...
def calculate_error(m: float, b: float, point: list[float]) -> float:
    x_point = point[0]
    y_point = point[1]
...
def calculate_all_error(m: float, b: float, datapoints) -> float:
    # total_error = []
    total_error = [calculate_error(m, b, points) for points in datapoints]
...
# * For every `b` value, take every `b` value in `possible_bs`
...
for m, b in itertools.product(possible_ms, possible_bs):
    error_val = calculate_all_error(m, b, datapoints)
    if error_val < smallest_error:
        smallest_error = error_val
        best_m, best_b = m, b

print(f"m: {best_m:.2f}")
print(f"b: {best_b:.2f}")
print(f"smallest_error: {smallest_error:.2f}")
# -
